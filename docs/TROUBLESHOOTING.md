# Troubleshooting — fabrik-test-chrome-extension
...
# Run tests
/opt/fabrik-test-chrome-extension/.venv/bin/pytest tests/ -v
```
...
| Import errors | Missing dependencies or wrong venv | `/opt/fabrik-test-chrome-extension/.venv/bin/pip install -r requirements.txt` |
| `externally-managed-environment` | Bare `pip install` on WSL/Debian | Always use `/opt/fabrik-test-chrome-extension/.venv/bin/pip`, never bare `pip` |
...
| `No module named 'fastapi'` | Venv not activated or deps missing | `source /opt/fabrik-test-chrome-extension/.venv/bin/activate && pip install -r requirements.txt` |
...
# Python environment
/opt/fabrik-test-chrome-extension/.venv/bin/python --version
/opt/fabrik-test-chrome-extension/.venv/bin/pip list
