...
| Key | Value |
|-----|-------|
| **Project** | `{fabrik-test-chrome-extension}` |
| **Port** | `8000` |
...
<!-- | **Docker-internal URL** | `http://{fabrik-test-chrome-extension}:8000` | -->
...
```bash
cd /opt/{fabrik-test-chrome-extension}
...
.venv/bin/uvicorn src.{package}.main:app --reload --port 8000
```
...
```bash
# Clone and configure
git clone {repo_url} && cd {fabrik-test-chrome-extension}
cp .env.example .env
...
<!-- Python API: /opt/{project}/.venv/bin/uvicorn src.{package}.main:app --reload --port 8000 -->
...
```bash
# Example workflow 3
curl -X DELETE http://localhost:8000/api/v1/{resource}/:id
```
...
{PROJECT}_URL = os.getenv("{PROJECT_NAME}_URL", "http://{fabrik-test-chrome-extension}:8000")
...
```typescript
const {PROJECT}_URL = process.env.{PROJECT_NAME}_URL ?? "http://{fabrik-test-chrome-extension}:8000";
...
export const {project} = {
  ...
  {primaryAction}: async (payload: Record<string, unknown>) => {
    ...
    if (!r.ok) throw new Error(`{fabrik-test-chrome-extension} ${r.status}: ${await r.text()}`);
    return r.json();
  },
};
```
...
```bash
curl -sf http://{fabrik-test-chrome-extension}:8000/health | jq .

curl -X POST http://{fabrik-test-chrome-extension}:8000/api/v1/{resource} \
  -H "Content-Type: application/json" \
  -d '{"field_1": "value"}'
```
...
services:
  your-service:
    environment:
      - {PROJECT_NAME}_URL=http://{fabrik-test-chrome-extension}:8000
    depends_on:
...
HTTP Request node:
  Method: POST
  URL: http://{fabrik-test-chrome-extension}:8000/api/v1/{resource}
...
```bash
0 2 * * * curl -sf -X POST http://{fabrik-test-chrome-extension}:8000/api/v1/maintenance/cleanup
```
...
```text
## {PROJECT_NAME}
- URL: http://{fabrik-test-chrome-extension}:8000 (Docker) | https://{project}.vps1.ocoron.com (external)
...
Env for callers: {PROJECT_NAME}_URL=http://{fabrik-test-chrome-extension}:8000
```
